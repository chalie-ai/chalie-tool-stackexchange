"""
Stack Exchange Tool Runner — Generates an inline HTML carousel card.

One question per slide showing score, tags, top answer excerpt, and link.
...
_ACCENT = "#F48024"          # Stack Overflow orange
_ACCENT_BG = "rgba(244,128,36,0.15)"
...
def _render_slide(result: dict, visible: bool) -> str:
    ...
    accepted = result.get("accepted_answer_id") is not None
    tags = result.get("tags", [])[:5]
    top_answer_body = result.get("top_answer_body") or ""
    display = "flex" if visible else "none"

    # Score + answer count badges
    accepted_html = ""
    if accepted:
        accepted_html = (
            f'<span style="background:rgba(95,179,86,0.2);color:#5FB356;'
            f'font-size:9px;font-weight:700;border-radius:3px;padding:1px 5px;'
            f'letter-spacing:0.04em;">ACCEPTED</span>'
        )
    ...
    meta_html = (
        ...
        f'\u25b2 {score}</span>'
        f'<span style="color:{_TEXT_TERTIARY};">{answer_count} answer{"s" if answer_count != 1 else ""}</span>'
        + accepted_html
        + '</div>'
    )
    ...
    if tags:
        tag_pills = "".join(
            f'<span style="background:{_TAG_BG};color:{_ACCENT};'
            f'font-size:10px;border-radius:3px;padding:1px 6px;">'
            f'{escape(t)}</span>'
            for t in tags
        )
        tags_html = f'<div style="display:flex;gap:4px;flex-wrap:wrap;margin-bottom:7px;">{tag_pills}</div>'

    # Top answer excerpt
    answer_html = ""
    if top_answer_body:
        excerpt = top_answer_body[:250] + ("\u2026" if len(top_answer_body) > 250 else "")
        answer_html = (
            f'<p style="font-size:12px;color:{_TEXT_SECONDARY};'
            f'line-height:1.5;margin:0 0 8px 0;'
            f'border-left:2px solid {_ACCENT};padding-left:8px;">'
            f'{escape(excerpt)}</p>'
        )
    ...
    return (
        ...
        + _LINK_ICON
        + '<span>View on Stack Exchange</span>'
        + '</a>'
        + '</div>'
    )
...
def _format_text(results: list, query: str, site: str) -> str:
    if not results:
        return (
            f'No Stack Exchange results found for "{query}" on {site}. '
            f'Try a different site or broaden the query.'
        )
    lines = [f'Stack Exchange results for "{query}" on {site}:']
    for i, r in enumerate(results, 1):
        lines.append(f"\n{i}. {r.get('title', '')}")
        lines.append(f"   Score: {r.get('score', 0)} | Answers: {r.get('answer_count', 0)}"
                     + (" | Accepted answer" if r.get("accepted_answer_id") else ""))
        if tags := r.get("tags", []):
            lines.append(f"   Tags: {', '.join(tags[:5])}")
        if body := r.get("top_answer_body", ""):
            lines.append(f"   Top answer: {body[:300]}")
        if url := r.get("url", ""):
            lines.append(f"   {url}")
    return "\n".join(lines)
